...
import com.example.ue5analyzer.domain.report.PdfExporter
...
fun ReportScreen(
    ...
    val pdfSaver = rememberLauncherForActivityResult(
        ...
        uri?.let {
            val success = pdfExporter.exportPdf(currentReport!!, it)
            scope.launch {
                if (success) {
                    snackbarHostState.showSnackbar(
                ...
                actions = {
                    if (currentReport != null) {
                        IconButton(
                            ...
                            Icon(Icons.Default.Download, "Export Markdown")
                        }
                        IconButton(
                            onClick = {
                                pdfSaver.launch("report_${System.currentTimeMillis()}.pdf")
                            }
                        ) {
        ...
        when {
            ...
            line.startsWith("  - ") || line.startsWith("\t- ") -> {
                ...
                while (i + 1 < lines.size) {
                    ...
                    if (nextIndent == indent || (nextIndent > indent && nextLine.trim()
                            .startsWith("- "))
                    ) {
                        i++
                ...
                if (nestedItems.isNotEmpty() && nestedItems.any { it.isNotEmpty() }) {
                    elements.add(
                        MarkdownElement.NestedUnorderedList(
                            indent,
                            nestedItems.filter { it.isNotEmpty() })
                    )
                }
            }
